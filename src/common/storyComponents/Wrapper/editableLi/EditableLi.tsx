...
export const EditableLi: FC<Props> = memo(({ idTodoList, taskId, title, onBlur }) => {
  ...
  return (
    <>
      {editableMode ? (
        <TextField value={value} autoFocus onChange={onChangeHandler} onBlur={onBlurHandler} variant={"standard"} />
      ) : (
        <SpanStyled onDoubleClick={onDoubleClickHandler}>{title}</SpanStyled>
      )}
    </>
  );
});
